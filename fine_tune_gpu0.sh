...
for year in 2012; do
        CUDA_VISIBLE_DEVICES=0 python finetuning_with_pos_neg_ctx.py \
                --retriever_model_id bert-base-uncased --pooling average \
                --train_data wmt_yearly_data_cut_tok/splitted/q_${year}_finetuning_data.jsonl \
                --eval_data wmt_yearly_data_cut_tok/splitted/q_${year}_dev_finetuning_data.jsonl \
                --loading_mode split \
                ...
                --output_dir ./checkpoint/contriever_wmt_${year}_finetune_moco_no_dev \
                --save_freq 5000 \
